...
{
    long long cost = 0;
    priority_queue<long long, vector<long long>, greater<long long>> priQueue; // Pri queue wt min heap
    ...
    {
        long long firstEle = priQueue.top();
        priQueue.pop();

        long long secondEle = priQueue.top();
        priQueue.pop();

        cost = cost + firstEle + secondEle;
        priQueue.push(firstEle + secondEle);
    }
    return cost;
}
